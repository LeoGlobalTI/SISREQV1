...
import { Bell, Info, CheckCircle, AlertTriangle, PlayCircle, Trash2, X, Clock, ExternalLink } from 'lucide-react';
...
export const NotificationBell: React.FC = () => {
  ...
  return (
    <div className="relative" ref={panelRef}>
      ...
      {isOpen && (
        <div className="absolute right-0 mt-3 w-80 bg-white rounded-2xl shadow-2xl border border-slate-200 z-[100] overflow-hidden flex flex-col animate-in fade-in slide-in-from-top-2 duration-200 max-h-[500px]">
          ...
          <div className="flex-1 overflow-y-auto custom-scrollbar">
            {notifications.length > 0 ? (
                <div className="divide-y divide-slate-50">
                    {notifications.map(n => (
                        <div 
                            ...
                            className={`p-4 hover:bg-slate-50 transition-colors cursor-pointer group relative ${!n.isRead ? 'bg-indigo-50/20' : ''}`}
                        >
                            {!n.isRead && <div className="absolute left-0 top-0 bottom-0 w-1 bg-indigo-600"></div>}
                            <div className="flex gap-3">
                                <div className={`shrink-0 p-2 rounded-lg bg-white border border-slate-100 shadow-sm ${!n.isRead ? 'ring-2 ring-indigo-500/10' : ''}`}>
                                    {getIcon(n.type)}
                                </div>
                                <div className="flex-1 min-w-0">
                                    <div className="flex items-center justify-between mb-0.5">
                                        <h4 className={`text-[11px] font-black uppercase tracking-tight truncate ${!n.isRead ? 'text-slate-900' : 'text-slate-500'}`}>
                                            {n.title}
                                        </h4>
                                    ...
                                    {n.requestId && (
                                        <div className="mt-2 flex items-center gap-1 text-[8px] font-black text-indigo-600 uppercase tracking-widest opacity-0 group-hover:opacity-100 transition-opacity">
                                            Ver Expediente <ExternalLink size={8} />
                                        </div>
                                    )}
                                </div>
                            </div>
                ...
                <div className="p-10 text-center flex flex-col items-center gap-3">
                    ...
                    <p className="text-[10px] font-black text-slate-300 uppercase tracking-widest">Sin alertas nuevas</p>
                </div>
            )}
          </div>
